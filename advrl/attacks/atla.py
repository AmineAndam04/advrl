...
class ATLAAttack:
    """
    ATLA (SA-RL) attack
    Paper: Robust Reinforcement Learning on State Observations with Learned Optimal Adversary
    Arxiv: https://arxiv.org/abs/2101.08452
    """
    ...
    def __init__(self, config, agent, low, high, **kwargs):
        ...
        if not config.train:
            self.load()
        self.hidden_atla = None

    def reset_hidden(self, index):
        if self.hidden_atla is not None:
            self.hidden_atla[0][:, index] = 0
            self.hidden_atla[1][:, index] = 0
    ...
    def mlp_collect_nsteps(
        ...
        for step in range(0, self.cfg_train.n_steps):
            with torch.no_grad():
                ...
                if self.cfg_train.squash:
                    adv_obs = observation + eps * action
                else:
                    adv_obs = observation + action
                ...
                agent_action, self.agent_h = self.agent.get_action(
                    obs=adv_obs, h=self.agent_h, deterministic=self.config.deterministic
                )
    ...
    def lstm_collect_nsteps(self):
        ...
        while len(alive_envs) > 0:
            with torch.no_grad():
                action, logprob, _, next_h = self.actor(obs=observation.unsqueeze(1), h=h)
                action = action.reshape(
                    (len(alive_envs),) + self.env.single_observation_space.shape
                )
                logprob = logprob.reshape((len(alive_envs),))
                if self.cfg_train.squash:
                    adv_obs = observation + eps * action
                else:
                    adv_obs = observation + action
                ...
                agent_action, agent_h = self.agent.get_action(
                    adv_obs.unsqueeze(1), h=agent_h, deterministic=self.config.deterministic
                )
                agent_action = agent_action.reshape(
    ...
    def get_perturbation(self, *, obs, h):
        obs_0 = obs.clone()
        perturbation, self.hidden_atla = self.actor.get_action(
            obs=obs_0, h=self.hidden_atla, deterministic=self.config.deterministic
        )
        perturbation = perturbation.reshape_as(obs_0)
        if self.actor.squash:
            adv_obs = obs_0 + self.eps * perturbation
        else:
            adv_obs = obs_0 + perturbation
        adv_obs = torch.clamp(adv_obs, min=obs_0 - self.eps, max=obs_0 + self.eps)
        adv_obs = torch.clamp(adv_obs, min=self.low, max=self.high)
