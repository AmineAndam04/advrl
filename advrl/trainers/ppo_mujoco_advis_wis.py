...
class PPO_MuJoCo_WIS:
    ...
    def setup_(self):
        ...
        self.run_name = f"ADV-WIS-PPO-MLP-{self.config.exp_name}__{time_token}"
        self.logger = Logger(
            mode=self.config.logger,
            wand_project=self.config.project_name,
            run_name=self.run_name,
            log_dir=f"{self.workdir}/{self.config.env_id}/advis_wis",
            config=self.config,
        )
        # Save config and command
        path = f"{self.workdir}/{self.config.env_id}/advis_wis/{self.run_name}"
        ...
        while self.current_step < self.config.total_timesteps:
            ...
            for _ in range(self.config.n_epochs):
                if self.config.full_traj:
                    prev_log_w = torch.zeros(self.config.num_envs)
                    prev_ret = torch.zeros(self.config.num_envs)
                for start in range(0, b_obs.shape[1], self.config.batch_size):
                    ...
                    if eps > 0 and self.config.rb_coef > 0:
                        ...
                        log_rho_masked = log_rho.masked_fill(~b_mask[:, start:end], 0.0)
                        log_w = log_rho_masked.sum(dim=-1)
                        if self.config.full_traj:
                            log_w += prev_log_w
                            prev_log_w = log_w.clone().detach()
                        log_w = log_w - log_w.max()
                        w = log_w.exp()
                        active_trajs = b_mask[:, start]
                        w = torch.where(active_trajs, w, torch.zeros_like(w))
                        ...
                        if self.config.full_traj:
                            rets += prev_ret
                            prev_ret = rets
                        wc_traj = (w * rets).sum() / (w.sum() + 1e-5)
                        act_traj = rets.sum() / active_trajs.sum()
                        adv_loss = self.adv_loss_fun(act_traj=act_traj, wc_traj=wc_traj)
                        if self.config.var_coef > 0 and active_trajs.sum() >= 2:
                            var = torch.sqrt(((w / (w.sum() + 1e-5)) ** 2).sum() + 1e-8)
                            is_vars.append(var.item())
                        wc_trajes.append(wc_traj.item())
                        act_trajes.append(act_traj.item())
                        adv_losses.append(adv_loss.item())
                        if self.config.conv_loss:
                            if self.config.var_coef > 0:
                                adv_loss = adv_loss + self.config.var_coef * var
                            loss = (1 - self.config.rb_coef) * loss + self.config.rb_coef * adv_loss
                        else:
                            loss += self.config.rb_coef * adv_loss
                            if self.config.var_coef > 0 and active_trajs.sum() >= 2:
                                loss += self.config.var_coef * var
            ...
            if eps > 0 and self.config.rb_coef > 0:
                ...
                if self.config.var_coef > 0 and len(is_vars) > 0:
                    training_metrics.update(
                        {
                            "w/var_is": np.mean(is_vars),
                        }
                    )
                if self.config.reg_coef > 0:
                    training_metrics.update(
    ...
    def chunk_cumsum(self, x, w):
        x_cumsum = torch.zeros_like(x)
        x_cumsum[:, 0] = x[:, 0]
        for i in range(1, x.size(-1)):
            ...
            #     last_ = torch.zeros_like(last_)
            x_cumsum[:, i] = x[:, i] + (1 - done) * x_cumsum[:, i - 1]
        return x_cumsum
