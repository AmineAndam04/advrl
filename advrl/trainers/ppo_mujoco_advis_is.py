...
class PPO_MuJoCo_IS:
    ...
    def setup_(self):
        ...
        self.run_name = f"ADV-IS-PPO-MLP-{self.config.exp_name}__{time_token}"
        self.logger = Logger(
            mode=self.config.logger,
            wand_project=self.config.project_name,
            run_name=self.run_name,
            log_dir=f"{self.workdir}/{self.config.env_id}/advis_is",
            config=self.config,
        )
        # Save config and command
        path = f"{self.workdir}/{self.config.env_id}/advis_is/{self.run_name}"
        ...
        while self.current_step < self.config.total_timesteps:
            ...
            for _ in range(self.config.n_epochs):
                if self.config.full_traj:
                    prev_log_w = torch.ones(self.config.num_envs)
                    prev_ret = torch.ones(self.config.num_envs)
                for start in range(0, b_obs.shape[1], self.config.batch_size):
                    ...
                    if eps > 0 and self.config.rb_coef > 0:
                        ...
                        if self.config.clip_logrho:
                            log_w = torch.clamp(
                                log_w, min=self.config.min_logrho, max=self.config.max_logrho
                            )
                        w = log_w.exp()
                        ...
                        if self.config.full_traj:
                            rets += prev_ret
                            prev_ret = rets
                        wc_traj = (w * rets).sum() / active_trajs.sum()
                        ...
                        if self.config.var_coef > 0 and active_trajs.sum() >= 2:
                            var = torch.sqrt(((w / (w.sum() + 1e-6)) ** 2).sum())
