...
class PAAD_PPO(Base_MLP_Trainer):
    ...
    def _setup(self):
        ...
        self.adv_env = VecEnv(
            [
                ...
                for i in range(self.config.paad.num_envs)
            ]
        )
        self.adv_actor = NETWORK_REGISTRY[self.config.paad.actor](
            in_dim=flatdim(self.env.single_observation_space),
            out_dim=flatdim(self.env.single_action_space),
            net_arch=ConfigDict(
                {
                    "pi_net": self.config.paad.pi_net,
                    "act_fun": self.config.paad.pi_act_fun,
                    "ortho_init": self.config.paad.ortho_init,
                    "orth_std": self.config.paad.orth_std,
                    "log_std_init": self.config.paad.log_std_init,
                },
            ),
        ).to(self.device)
        self.adv_critic = NETWORK_REGISTRY[self.config.paad.critic](
            in_dim=flatdim(self.env.single_observation_space),
            net_arch=ConfigDict(
                {
                    "vf_net": self.config.paad.vf_net,
                    "act_fun": self.config.paad.vf_act_fun,
                    "ortho_init": self.config.paad.ortho_init,
                    "orth_std": self.config.paad.orth_std,
                }
            ),
        ...
        self.adv_actor_optimizer = optimizer(
            self.adv_actor.parameters(), lr=self.config.paad.lr_actor
        )
        self.adv_critic_optimizer = optimizer(
            self.adv_critic.parameters(), lr=self.config.paad.lr_critic
        )

        self.eps_schedule = set_scheduler(config=self.config.paad)
        #! END: adv

    def _reset_rollout(self, config=None):
        config = config or self.config
        obs = torch.zeros(
            (config.n_steps, config.num_envs) + self.env.single_observation_space.shape
        ).to(self.device)
        actions = torch.zeros(
            (config.n_steps, config.num_envs) + self.env.single_action_space.shape
        ).to(self.device)
    ...
    def collect_nsteps(
        ...
        for step in range(0, self.config.n_steps):
            #! START: adv
            with torch.no_grad():
                mu_dir, std_dir, *_ = self.adv_actor.get_mu_and_std(obs=observation)
            observation = self.paad_perturbation(
                observation=observation, eps=eps, mu_dir=mu_dir, std_dir=std_dir
            )
            #! END: adv
            with torch.no_grad():
    ...
    def paad_perturbation(self, observation, eps, mu_dir, std_dir):
        step_size = self.config.paad.step_size
        if step_size < 0:
            step_size = eps / self.config.paad.num_iter
        delta = torch.zeros_like(observation).uniform_(-step_size, step_size)
        # As implemented in the paper (Don't forget to comment in the loop)
        # with torch.no_grad():
        #     mu_0, _, *_ = self.actor.get_mu_and_std(obs=observation)
        for _ in range(self.config.paad.num_iter):
            delta = delta.clone().detach().requires_grad_(True)
            mu, std, *_ = self.actor.get_mu_and_std(obs=observation + delta)
            if self.config.paad.deterministic_victim:
                loss = ((mu - mu_dir) ** 2).sum()
            else:
                loss = kl.kl_divergence(p=Normal(mu, std), q=Normal(mu_dir, std_dir)).sum()
            # As implemented in the paper (Don't forget to uncomment before the loop)
            # loss = (
            #     torch.nn.functional.cosine_similarity(mu - mu_0, mu_dir, dim=-1)
            #     + torch.norm(mu - mu_0, p=2, dim=-1)
            # ).mean()
            loss.backward()
            grad = delta.grad
            delta = torch.clamp(delta - step_size * torch.sign(grad), -eps, eps)
        return torch.clamp(observation + delta.detach().clone(), min=self.low, max=self.high)
    ...
    def collect_adv_nsteps(
        ...
        obs, actions, logprobs, rewards, dones, values = self._reset_rollout(
            config=self.config.paad
        )
        if self.config.paad.adv_scheduled:
            eps = self.eps_schedule(current_t=self.current_step / self.config.total_timesteps)
        else:
            eps = self.config.paad.eps
        for step in range(0, self.config.paad.n_steps):
            with torch.no_grad():
                mu_dir, std_dir, *_ = self.adv_actor.get_mu_and_std(obs=observation)
            ...
            adv_obs = self.paad_perturbation(
                observation=observation, eps=eps, mu_dir=action, std_dir=std_dir
            )
            with torch.no_grad():
                agent_action, *_ = self.actor.get_action(
                    obs=adv_obs, deterministic=self.config.paad.deterministic_victim
                )
            ...
            logprobs[step] = logprob
            observation = torch.as_tensor(next_obs).to(self.device)
            self.adv_current_step += self.config.paad.num_envs
            for info in infos:
                if "episode" in info:
                    ep_rd = info["episode"]["r"]
                    ep_len = info["episode"]["l"]
        ...
        returns, advantages = self.compute_advantages_and_returns(
            values, last_value, rewards, dones, config=self.config.paad
        )
        b_obs = obs.reshape((-1,) + self.env.single_observation_space.shape)
        b_logprobs = logprobs.reshape(-1)
        b_actions = actions.reshape((-1,) + self.env.single_action_space.shape)
