...
class PPO_MuJoCo_WPDIS:
    ...
    def setup_(self):
        ...
        self.run_name = f"ADV-WPD-PPO-MLP-{self.config.exp_name}__{time_token}"
        self.logger = Logger(
            mode=self.config.logger,
            wand_project=self.config.project_name,
            run_name=self.run_name,
            log_dir=f"{self.workdir}/{self.config.env_id}/advis_wpd",
            config=self.config,
        )
        # Save config and command
        path = f"{self.workdir}/{self.config.env_id}/advis_wpd/{self.run_name}"
        ...
        while self.current_step < self.config.total_timesteps:
            ...
            for _ in range(self.config.n_epochs):
                if self.config.full_traj:
                    prev_log_rho = torch.zeros(self.config.num_envs)
                for start in range(0, b_obs.shape[1], self.config.batch_size):
                    ...
                    if eps > 0 and self.config.rb_coef > 0:
                        ...
                        log_rho_max_per_step, _ = log_rho.max(dim=0, keepdim=True)
                        log_rho_shifted = log_rho - log_rho_max_per_step
                        log_w = log_rho_shifted - torch.log(
                            log_rho_shifted.exp().sum(dim=0, keepdim=True)
                        )
                        w = log_w.exp()
                        # Weighted IS degenerates when only 1 episode is active
                        num_active_envs = b_mask[:, start:end].sum(dim=0)
                        if torch.any(num_active_envs < 2):
                            single_episode = (num_active_envs == 1).unsqueeze(0)
                            w = torch.where(
                                single_episode & b_mask[:, start:end],
                                torch.exp(torch.clamp(log_rho, min=-6, max=6)),
                                w,
                            )
