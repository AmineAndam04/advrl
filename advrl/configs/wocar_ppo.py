...
def get_config():
    config = ConfigDict()
    config.name = "wocar_pop"  # config id
    # Neural nets
    ...
    config.wc_critic = "qnet_continuous_mlp"
    config.wc_net = (64, 64)
    config.wc_act_fun = "tanh"
    config.lr_wc = 0.0001
    config.wc_n_epochs = 1
    config.wc_target_iter = 50
    config.wc_polyak = 0.001
    ...
    config.normalize_qvals = True
    config.use_state_importance = True
    config.state_importance_iter = 10  # Number of iterations for state-importance weight
    config.state_importance_lr = 0.001  # Step sizes for state-importance weight
    config.reg_solver = "relax"  # solver for value-enhanced state regularization
    ...
    config.rb_coef = 0.01  # robust loss coefficient
    config.reg_coef = 0.05
    ...
    config.verbose = 0
    config.algo_name = "wocar"
    config.prefix_run_name = "WocaR-PPO"
    # Evaluation
