...
def get_config():
    config = ConfigDict()
    config.name = "advis_is_ppo_with_mlp"  # config id
    # Neural nets
    ...
    config.var_coef = -1.0  # variance coef
    config.reg_coef = -1.0  # SA-reg loss
    config.rb_loss = "relu"  # relu, mse, mae, wc
    config.discount_traj = False
    config.full_traj = False
    config.clip_logrho = False
    config.min_logrho = -2.0
    config.max_logrho = 2.0
    config.conv_loss = False  # conv loss (1-λ)*L_PPO + λ*L_reg else L_PPO + λ*L_reg
    # Training
