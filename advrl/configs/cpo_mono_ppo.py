...
def get_config():
    config = ConfigDict()
    config.name = "cpo_mono_ppo"  # config id
    # Neural nets
    ...
    config.advantage_coef = 1.0
    config.balance_grad = False
    config.use_pid = False
    config.lag_optim = "Adam"
    config.lag_init = 0.0
    ...
    config.verbose = 0
    config.algo_name = "cpo_mono"
    config.prefix_run_name = "CPO-MONO-PPO"
    # Evaluation
